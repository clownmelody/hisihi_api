...
from herovii.service.information_flow import get_information_flow_banner_service, get_information_flow_content_service, \
    ...
    get_information_flow_content_service_v2_9, search_information_flow_content_service_v2_9, \
    get_information_flow_content_service_v2_9_6
...
@api.route('/2.96/content', methods=['GET'])
@cache.cached(timeout=120, key_prefix=make_cache_key)
def get_information_flow_content_v2_9_6():
    request_json = request.get_json(force=True, silent=True)
    page, per_page = parse_page_args(request_json)
    if request_json is None or 'type' not in request_json.keys():
        information_type = -1
    ...
    if 'type' in request.args:
        information_type = int(request.args.get('type'))
    total_count, data_list = get_information_flow_content_service_v2_9_6(uid, information_type, page, per_page)
    result = {
        'total_count': total_count,
        'data': data_list
    }
...
@api.route('/2.9/search', methods=['GET'])
def search_information_flow_content_v2_9():
    request_json = request.get_json(force=True, silent=True)
    page, per_page = parse_page_args(request_json)
    if not hasattr(g, 'user'):
        uid = 0
    ...
    if 'keywords' in request.args:
        keywords = request.args.get('keywords')
    total_count, data_list = search_information_flow_content_service_v2_9(uid, keywords, page, per_page, 2.9)
    result = {
        'total_count': total_count,
        'data': data_list
    }
