...
def get_overseas_study_university_info_service_v2_9_5(user_id, uid):
    university = University.query.get(uid)
    if not university:
        raise NotFound(error_code=5008, error='大学信息不存在')
    graduate_majors_array = university.graduate_majors.split("#")
...
def is_university_favorite(uid, university_id):
    is_favorite = db.session.query(Favorite).filter(Favorite.row == university_id,
